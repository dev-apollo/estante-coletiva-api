...
export async function updateUsuario(id, usuario) {
    const [result] = await db.query("UPDATE usuario SET nome = ?, sobrenome = ?, email = ?, senha = ?, data_cadastro = ?, usuario_status = ?, cidade_id = ? WHERE id = ?", [usuario.nome, usuario.sobrenome, usuario.email, usuario.senha, usuario.data_cadastro, usuario.usuario_status, usuario.cidade_id, id]);
    return result.affectedRows > 0;
}